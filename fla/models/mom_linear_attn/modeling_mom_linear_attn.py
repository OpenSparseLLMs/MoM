...
from fla.layers.mom_linear_attn import MomLinearAttention
from fla.models.mom_linear_attn.configuration_mom_linear_attn import \
...
class MomLinearAttentionMLP(nn.Module):
...
class MomLinearAttentionBlock(nn.Module):
    def __init__(self, config: MomLinearAttentionConfig, layer_idx: int):
        super().__init__()
        ...
        self.mlp = MomLinearAttentionMLP(
            hidden_size=config.hidden_size,
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits = self.attn(hidden_states)
...
class MomLinearAttentionPreTrainedModel(PreTrainedModel):

    config_class = MomLinearAttentionConfig
...
@dataclass
class MomLAOutputWithPast(BaseModelOutputWithPast):
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MomLinearAttentionModel(MomLinearAttentionPreTrainedModel):

    def __init__(self, config: MomLinearAttentionConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([MomLinearAttentionBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
...
@dataclass
class MomLACausalLMOutputWithPast(CausalLMOutputWithPast):
    aux_loss: Optional[torch.FloatTensor] = None
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MomLinearAttentionForCausalLM(MomLinearAttentionPreTrainedModel, GenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MomLinearAttentionModel(config)
        self.vocab_size = config.vocab_size
