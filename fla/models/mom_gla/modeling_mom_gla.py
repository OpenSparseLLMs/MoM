...
from fla.layers.mom_gla import MomGatedLinearAttention
from fla.models.mom_gla.configuration_mom_gla import MomGLAConfig
...
class MomGLABlock(nn.Module):
    def __init__(self, config: MomGLAConfig, layer_idx: int):
        super().__init__()
        ...
        if config.attn is not None and layer_idx in config.attn['layers']:
            ...
            self.attn = MomGatedLinearAttention(
                ...
                single_expert=config.single_expert
            )
...
class MomGLAPreTrainedModel(PreTrainedModel):

    config_class = MomGLAConfig
    supports_gradient_checkpointing = True
    _no_split_modules = ['MomGLABlock']
...
@dataclass
class MomGLAOutputWithPast(BaseModelOutputWithPast):
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MomGLAModel(MomGLAPreTrainedModel):

    def __init__(self, config: MomGLAConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([MomGLABlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        if output_attentions:
            warnings.warn("`MomGLAModel` does not `output_attentions` now, setting it to `False`.")
...
@dataclass
class MomGLACausalLMOutputWithPast(CausalLMOutputWithPast):
    aux_loss: Optional[torch.FloatTensor] = None
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MomGLAForCausalLM(MomGLAPreTrainedModel, GenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MomGLAModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        return MomGLACausalLMOutputWithPast(
            loss=loss,
            logits=logits,
