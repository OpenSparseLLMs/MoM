...
from fla.layers.mom_gsa import MomGatedSlotAttention
from fla.models.mom_gsa.configuration_mom_gsa import MomGSAConfig
...
class MomGSABlock(nn.Module):
    def __init__(self, config: MomGSAConfig, layer_idx: int):
        super().__init__()
        ...
        if config.attn is not None and layer_idx in config.attn['layers']:
            ...
            self.attn = MomGatedSlotAttention(
                hidden_size=config.hidden_size,
...
class MomGSAPreTrainedModel(PreTrainedModel):

    config_class = MomGSAConfig
    supports_gradient_checkpointing = True
    _no_split_modules = ['MomGSABlock']
...
@dataclass
class MomGSAOutputWithPast(BaseModelOutputWithPast):
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MomGSAModel(MomGSAPreTrainedModel):

    def __init__(self, config: MomGSAConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([MomGSABlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
...
@dataclass
class MomGSACausalLMOutputWithPast(CausalLMOutputWithPast):
    aux_loss: Optional[torch.FloatTensor] = None
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MomGSAForCausalLM(MomGSAPreTrainedModel, GenerationMixin):
    ...
    def get_model_weight_stats(self):
        max_value = -float('inf')
        min_value = float('inf')
        max_name = None
        min_name = None
        ...
        for name, param in self.named_parameters():
            if param.requires_grad:
                ...
                if param_max > max_value:
                    max_value = param_max
                    max_name = name
                
                if param_min < min_value:
                    min_value = param_min
                    min_name = name
        print(f"Max value: {max_value} in {max_name}")
        print(f"Min value: {min_value} in {min_name}")
