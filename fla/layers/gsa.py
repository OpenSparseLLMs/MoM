...
class GatedSlotAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # def random_pad_length(q, k, v, f, s):
        #     seq_len = q.shape[1]
        #     rand_ratio = torch.rand((1)).item()
        #     pad_len = int(seq_len * rand_ratio)
        #     pad_q = torch.zeros((q.shape[0], pad_len, q.shape[2], q.shape[3]), dtype=q.dtype, device=q.device)
        #     pad_k = torch.zeros((k.shape[0], pad_len, k.shape[2], k.shape[3]), dtype=k.dtype, device=k.device)
        #     pad_v = torch.zeros((v.shape[0], pad_len, v.shape[2], v.shape[3]), dtype=v.dtype, device=v.device)
        #     pad_f = torch.zeros((f.shape[0], pad_len, f.shape[2], f.shape[3]), dtype=f.dtype, device=f.device)
        #     pad_s = torch.zeros((s.shape[0], pad_len, s.shape[2], s.shape[3]), dtype=s.dtype, device=s.device)

        #     q_expand = torch.cat((q, pad_q), dim=1)
        #     k_expand = torch.cat((k, pad_k), dim=1)
        #     v_expand = torch.cat((v, pad_v), dim=1)
        #     f_expand = torch.cat((f, pad_f), dim=1)
        #     s_expand = torch.cat((s, pad_s), dim=1)

        #     return q_expand, k_expand, v_expand, f_expand, s_expand

        # seq_len = q.shape[1]
        # q, k, v, f, s = random_pad_length(q, k, v, f, s)

        recurrent_state = last_state['recurrent_state'] if last_state is not None else None
        offsets = kwargs.get('offsets', None)
        if mode == 'fused_recurrent':
            o, state_e = fused_recurrent_gsa(
                q=q,
                k=k,
                v=v,
                s=s,
            ...
            # infer_time = (time.time() - begin_time) * 1000
