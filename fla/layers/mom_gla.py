...
class MomGatedLinearAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.single_expert:
            ...
            self.k_proj =  nn.ModuleList([nn.Linear(self.hidden_size, self.key_dim_per_group, bias=False) for _ in range(self.num_experts)])
            self.v_proj = nn.ModuleList([nn.Linear(self.hidden_size, self.value_dim_per_group, bias=False) for _ in range(self.num_experts)])
            if self.shared_mem:
                self.shared_k = nn.Linear(hidden_size, self.key_dim_per_group, bias=False)
                self.shared_v = nn.Linear(hidden_size, self.value_dim_per_group, bias=False)
